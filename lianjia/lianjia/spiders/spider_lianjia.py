...
class SpiderLianjiaSpider(scrapy.Spider):
    name = 'spider_lianjia'
    allowed_domains = ['https://nj.lianjia.com']
    xiaoqu_url = "https://nj.lianjia.com/xiaoqu/"
    region = {'gulou': '鼓楼',
              'jianye': '建邺',
              'qinhuai': '秦淮',
              'xuanwu': '玄武',
    ...
    def parse_xiaoqu(self,response):
        ...
        for xq_name in xiaoqu_list:
            ...
            yield Request(url=url,callback=parse_chengjiao,meta={'xq_name':xq_name,'region':response.meta['region']})
    ...
    def parse_chengjiao(self,response):
        ...
        for i in range(int(total_pages)):
            url="https://nj.lianjia.com/chengjiao/pg{}rs{}/".format(str(i+1),quote(xq_name))
            yield Request(url=url,callback=self.parse_content,meta={"region":response.meta["region"]})
