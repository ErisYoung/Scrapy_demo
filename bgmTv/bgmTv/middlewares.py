...
class MyUserAgentMiddleware(UserAgentMiddleware):
    ...
    def from_crawler(cls, crawler):
        return  cls(
            agent=crawler.settings.get('USER_AGENTS')
        )

    def process_request(self, request, spider):
        random_agent=random.choice(self.agent)
...
class IpproxyMiddleWare(object):
    ...
    def get_random_proxy(self):
        ...
        while 1:
            with open('D:\\s-data\\note\\pro\\python\\pratice\\scrapy\\ipproxy\\proxies.txt','r') as f:
                proxies=f.readlines()
                if proxies:
                    break
                else:
...
class BgmtvSpiderMiddleware(object):
...
class BgmtvDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
